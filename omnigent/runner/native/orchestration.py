...
async def _launch_codex_native_tui(
    ...
    *,
    ...
    harness_id = "localdex-native" if localdex else "codex-native"
    terminal_name = "localdex" if localdex else "codex"
    terminal_role = LOCALDEX_NATIVE_TERMINAL_ROLE if localdex else CODEX_NATIVE_TERMINAL_ROLE
    _codex_cmd_override = (
    ...
    codex_launch_args = resolve_harness_args(
        harness_id, tuple(codex_remote_args), cfg=_codex_harness_cfg
    )
    loggable_launch_args = shlex.join(redact_codex_launch_args(codex_launch_args))
    _logger.info(
        "Codex-compatible terminal launch: session=%s harness=%s command=%s codex_cli_version=%s resume=%s args=%s",
        session_id,
        harness_id,
        codex_command,
    ...
    terminal_registry = getattr(resource_registry, "terminal_registry", None)
    terminal_instance = (
        terminal_registry.get(session_id, terminal_name, "main")
        if terminal_registry is not None
        else None
    )
...
async def _auto_create_codex_terminal(
    ...
    *,
    ...
    if launch_config.external_session_id is not None:
        _logger.info(
            "Codex-compatible native input ready",
            extra=debug_event(
                "native_input_ready",
                session_id=session_id,
